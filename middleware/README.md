# The `wasi:http/middleware` World #

To fetch latest wit bindings: `wkg wit fetch`
...
2. Import handler.handle (the downstream handler)
...
The full [world](https://github.com/WebAssembly/WASI/blob/main/proposals/http/wit-0.3.0-draft/worlds.wit):
```wit
...
That means:
1. You export wasi:http/handler.handle
2. You import wasi:http/handler.handle (same interface, downstream)

In Rust bindings, these become two different traits:
- the one you implement
- the one you call

# How is this wired at runtime? #
...
client/network
    ↓
LoggingMiddleware (this component)
    ↓
Actual HTTP service (router, app, proxy, etc.)
```
